...
export default function Navbar({ children }: NavbarProps) {
  ...
  const navbarContent = (
    <header className="sticky top-0 z-50 w-full h-[72px] min-h-[72px] border-b border-gray-200/50 dark:border-white/10 backdrop-blur-sm   will-change-transform transform-gpu">
      ...
      <div className="mx-auto flex w-full h-full max-w-9xl items-center justify-between gap-2 sm:gap-4 px-2 sm:px-4 lg:px-6 overflow-x-hidden">
        ...
        <div className="flex items-center gap-3">
          <div
            ...
            className="group flex aspect-square size-10 items-center justify-center rounded-xl border   backdrop-blur-sm cursor-pointer transition-all duration-200 dark:  dark:"
          ...
          >
            <AiFillProduct className="text-2xl text-rose-600 dark:text-rose-400 transition-transform group-hover:scale-110 drop-" />
          </div>
          <h1 className="text-lg font-semibold tracking-tight text-gray-900 dark:text-white lg:text-xl transition-all duration-300 ease-in-out cursor-pointer">
            Stock Inventory
          </h1>
        </div>
          ...
          {navItems.map((item) => {
              ...
              return (
                <DropdownMenu key={item.label}>
                  ...
                  <DropdownMenuContent
                    align="start"
                    sideOffset={2}
                    className="w-48 border border-white/10 dark:border-white/10 backdrop-blur-sm   text-gray-900 dark:text-white"
                  >
                    {item.dropdownItems.map((sub) => (
                      <DropdownMenuItem
                        key={sub.path}
                        onSelect={() => handleNavigation(sub.path)}
                        className="text-gray-700 dark:text-muted-foreground hover:text-gray-900 dark:hover:text-foreground cursor-pointer"
                      >
                        {sub.label}
                      </DropdownMenuItem>
                    ))}
            ...
            return (
              <Button
                ...
                className="text-sm font-medium text-gray-700 dark:text-muted-foreground will-change-[background,box-shadow,color] transition-[background-image,box-shadow,color] duration-300 ease-in-out hover:text-sky-600 dark:hover:text-foreground hover:backdrop-blur-sm  dark: rounded-md px-3 py-2"
              >
                {item.label}
              </Button>
            );
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          {isCheckingAuth ? (
            ...
            <div className="relative h-8 w-8 sm:h-10 sm:w-10 rounded-full border  backdrop-blur-sm animate-pulse flex items-center justify-center">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  className="relative h-10 w-10 min-h-10 min-w-10 rounded-full border-2 dark:border-white/20 backdrop-blur-sm dark:hover:border-white/30 transition-all duration-200   ring-2 ring-sky-400/30 dark:ring-white/20 hover:ring-sky-400/50 dark:hover:ring-white/30 p-0 overflow-hidden focus-visible:outline-none focus:outline-none focus-visible:ring-0 focus:ring-0"
                >
                  {isCheckingAuth ? (
          ...
          <div className="flex items-center lg:hidden">
            ...
            <Button
              ...
              className="h-8 w-8 sm:h-10 sm:w-10 text-gray-900 dark:text-foreground hover:backdrop-blur-sm transition-all duration-300 ease-in-out"
            >
              {isMobileMenuOpen ? (
      ...
      {isMobileMenuOpen && (
        <div
          ...
          className="xl:hidden border-t border-white/10 dark:border-white/10 backdrop-blur-sm max-h-[calc(100vh-3.5rem)] overflow-y-auto"
        >
          <div className="mx-auto w-full max-w-9xl px-2 sm:px-4 lg:px-6 sm:py-6 space-y-3">
            ...
            <div className="flex items-center gap-3 px-2 py-2">
              {isCheckingAuth ? (
                ...
                avatarUrl && (
                  <div className="relative flex h-10 w-10 items-center justify-center rounded-full border-2 dark:border-white/20 backdrop-blur-sm overflow-hidden ring-2 ring-sky-400/30 dark:ring-white/20 ">
                    <Image
                      src={avatarUrl}
                      alt={user?.name || "User"}
                      width={40}
              ...
              {navItems.map((item) => {
                      ...
                      <div className="pl-4 space-y-1">
                        {item.dropdownItems.map((sub) => (
                          <Button
                            key={sub.path}
                            variant="ghost"
                            className="w-full justify-start text-gray-600 dark:text-muted-foreground hover:text-gray-900 dark:hover:text-foreground hover:backdrop-blur-sm transition-all duration-300 ease-in-out px-3 py-2.5 h-auto min-h-[40px] text-sm"
                            onClick={() => {
                ...
                return (
                  <Button
                    key={item.path}
                    variant="ghost"
                    className="w-full justify-start text-gray-700 dark:text-muted-foreground hover:text-gray-900 dark:hover:text-foreground hover:backdrop-blur-sm transition-all duration-300 ease-in-out px-3 py-3.5 h-auto min-h-[44px]"
                    onClick={() => handleNavigation(item.path)}
                  >
                    {item.label}
                  </Button>
